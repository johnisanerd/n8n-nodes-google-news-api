...
import { ACTOR_ID } from '../ApifyGoogleNews.node';
import { buildActorInput } from '../ApifyGoogleNews.properties';
...
/**
 * Shape a single news article according to the chosen Output mode.
 ...
 * `news_results` array. Flatten to one article per n8n item.
 */
function flattenResults(items: any[]): Record<string, any>[] {
	const results: Record<string, any>[] = [];
	for (const item of items) {
		const all = Array.isArray(item?.news_results) ? item.news_results : [];
		for (const r of all) {
			results.push(r);
		}
	}
